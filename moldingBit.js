...
$.each(UnitPopup.unit_data, function(a,b) {
    ...
    function cataOpenPlace() {
        ...
        try {
            ...
            if (e instanceof UserError) {
                UI.ConfirmationBox(e.message + '\nDo you want to go to the next report?', [{
                    text: 'Oui',
    ...
    function errorTryNextVillage(message) {
        ...
        UI.ConfirmationBox(message + '\nDo you want to move on to the next village?', [{
            text: 'Oui',
            callback: function() {
    ...
    function fillCata() {
        ...
        if ($("#unit_input_catapult").val() === "") {
            ...
            if (!cataInfo) {
                UI.SuccessMessage("It's all been destroyed.");
            ...
            if (cataInfo.nextLevel === 0) {
                UI.SuccessMessage("Last level of the building " + buildings[cataInfo.id].name);
                // the building will be destroyed - send some spies if we have enough
                if (numberOfUnits('spy') >= numScoot) {
                    $("#unit_input_spy").val(numScoot);
                }
            } else {
                UI.InfoMessage("Building reduction " + buildings[cataInfo.id].name + " at the level " + cataInfo.nextLevel);
            }
            ...
            window.setTimeout(() => { $('#target_attack').click(); }, 1000);
    ...
    function sendCata() {
      ...
      $('#troop_confirm_submit').prop('disabled', true);*/
        ...
        window.setTimeout(() => { $('#troop_confirm_submit').click(); }, 1000);
        //$('#troop_confirm_submit').prop('disabled', true);
    }
    ...
    function farmSaveReportLinks() {
        ...
        if (!window.reportsSaved) {
            ...
            UI.InfoMessage("List of saved reports. Total of " + links.length + " reports.");
    ...
    function run() {
        try {
            if (game_data.screen === "report" && location.href.indexOf('view') > 0) {
                ...
                UI.ErrorMessage('Launch the script from a spy report or from the looting assistant.');
            }
        } catch (e) {
            if (e instanceof UserError) {
                ...
                // likely a bug
                UI.ErrorMessage("An error has occurred " + e);
                var el = $('<div/>', {
                    class: 'info_box',
                    id: 'cata_barb_error',
